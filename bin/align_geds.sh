...
# Static files (common for all genes)
GEDS_FILE="$OUTPUT_DIR/hg38.geds"
GEDMAP_POS2FA_FILE="$OUTPUT_DIR/hg38.geds.2fa"
GEDMAP_INDEX_FILE="$OUTPUT_DIR/hg38.geds.min"
...
if [[ ! -f "$GEDS_FILE" || ! -f "$GEDMAP_POS2FA_FILE" ]]; then
    echo "Error: Static GEDS or 2FA file not found. Please ensure $GEDS_FILE and $GEDMAP_POS2FA_FILE are present."
    exit 1
fi

# Check if the minimizer index file exists, if not, create it
if [[ ! -f "$GEDMAP_INDEX_FILE" ]]; then
    echo "Running GEDMAP index for all genes..."
    ../gedmap/gedmap index "$GEDS_FILE" -2fa "$GEDMAP_POS2FA_FILE" -o "$GEDMAP_INDEX_FILE" -t 8

    if [[ $? -ne 0 ]]; then
        echo "Error: GEDMAP index failed."
        exit 1
    fi
else
    echo "Index file already exists: $GEDMAP_INDEX_FILE"
fi
...
while IFS=, read -r chromosome start end gene_id gene_name snp_density rna_type; do
    ...
    if [[ -f "$fastq_file" ]]; then
        # Align: Align FASTQ to GEDS (only if the SAM file doesn't exist)
        if [[ ! -f "$sam_file" ]]; then
